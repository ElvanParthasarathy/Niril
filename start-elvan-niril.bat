@echo off
title Elvan Niril Server
cd /d "%~dp0"

where node >nul 2>nul
if %errorlevel% neq 0 (
    echo Node.js not found. Run "Install Elvan Niril.bat" first.
    pause
    exit /b 1
)
...
echo  Elvan Niril running at http://localhost:%ACTIVE_PORT%
...
:opendefault
echo  Elvan Niril running at http://localhost:3001

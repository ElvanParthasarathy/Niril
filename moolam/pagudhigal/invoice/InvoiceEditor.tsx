                  ...
                  <Grid size={{ xs: 12, md: 6 }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 1 }}>
                      {PDF_STYLES.map(s => (
                        <Chip key={s.id} 
                          label={s.label} 
                          color={(invoiceOptions.pdfStyle || 'classic') === s.id ? "primary" : "default"} 
                          variant={(invoiceOptions.pdfStyle || 'classic') === s.id ? "filled" : "outlined"} 
                  ...
                  <Grid size={{ xs: 12, md: 6 }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
                      <Box component="button" type="button" title="Auto (match invoice type)"
                        ...
                        {!invoiceOptions.accentColor && <Box component="span" sx={{ position: 'absolute', inset: '3px', borderRadius: '50%', border: '2px solid white' }} />}
                      </Box>
                      {ACCENT_PRESETS.map(p => (
                        <Box key={p.color} component="button" type="button" title={p.label}
                          sx={{ width: 28, height: 28, borderRadius: '50%', backgroundColor: p.color, border: invoiceOptions.accentColor === p.color ? '2.5px solid #334155' : '2px solid #cbd5e1', cursor: 'pointer', position: 'relative' }}
                          onClick={() => setInvoiceOptions(prev => ({ ...prev, accentColor: p.color }))}>
                          {invoiceOptions.accentColor === p.color && <Box component="span" sx={{ position: 'absolute', inset: '3px', borderRadius: '50%', border: '2px solid white' }} />}
                        </Box>
                      ))}
                    </Box>
                  </Grid>
                ...
                <Grid container spacing={3}>
                  {[
                    { group: 'Header & branding', items: [
                      ['showLogo', 'Logo'],
                      ['showniruvanathinPeyar', 'Business name'],
                    ...
                    <Grid size={{ xs: 12, sm: 6, md: 4 }} key={section.group}>
                      ...
                      <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                        {section.items.map(([key, label]) => {
                          ...
                          return (
                            <FormControlLabel key={key} control={<Checkbox size="small" checked={checked} onChange={() => toggleOption(key)} />} label={<Typography variant="body2">{label}</Typography>} />
                          );
                        })}
                      </Box>
                    </Grid>

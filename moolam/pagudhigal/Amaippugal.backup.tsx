...
import { 
  ...
  List, ListItem, ListItemText, ListItemSecondaryAction, Tabs, Tab, Fab, Avatar
} from '@mui/material';
...
export default function Amaippugal({ onSaved }) {
  ...
  const renderDetailView = () => {
    ...
    if (currentView === 0) { 
        content = (
            <Box sx={{ p: 0, mb: 4 }}>
                <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 1, px: 1 }}>{t('companyDetailsTitle')}</Typography>
                <Paper elevation={0} sx={{ p: 2, border: '1px solid', borderColor: 'divider', borderRadius: 2, cursor: 'pointer', '&:hover': { bgcolor: 'action.hover' } }} onClick={() => setIsEditingProfile(true)}>
  ...
  return (
    <div className="s2-page-view" style={isMobile ? { position: 'relative', overflow: 'hidden', height: 'calc(100vh - 60px)', paddingTop: 16, fontFamily: '"Elvan Sans", sans-serif' } : { paddingTop: 0, fontFamily: '"Elvan Sans", sans-serif' }}>
        <Dialog open={isEditingProfile} onClose={() => setIsEditingProfile(false)} fullScreen>
          <ProfileEditor 

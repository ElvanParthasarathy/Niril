...
const DATA_DIR = path.join(__dirname, 'tharavu');
...
const PORT_FILE = path.join(__dirname, 'tharavu', 'port.txt');
...
// BILLS
// ========================
app.get('/api/pattiyalkal', (req, res) => {
  const bills = readAllFromDir('pattiyalkal');
  bills.sort((a, b) => new Date(b.invoiceDate) - new Date(a.invoiceDate));
  res.json(bills);
});

app.post('/api/pattiyalkal', (req, res) => {
  const bill = req.body;
  if (!bill || !bill.id) return res.status(400).json({ error: 'Bill must have an id' });
  const filePath = path.join(DATA_DIR, 'pattiyalkal', safeFileName(bill.id) + '.json');
  writeJSON(filePath, bill);
  res.json({ success: true });
});

app.delete('/api/pattiyalkal/:id', (req, res) => {
  const filePath = path.join(DATA_DIR, 'pattiyalkal', safeFileName(req.params.id) + '.json');
  deleteFile(filePath);
  res.json({ success: true });
});
...
// CLIENTS
// ========================
app.get('/api/vanigargal', (req, res) => {
  const clients = readAllFromDir('vanigargal');
  clients.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
  res.json(clients);
});

app.post('/api/vanigargal', (req, res) => {
  const client = req.body;
  if (!client.id) client.id = 'cli_' + Date.now();
  const filePath = path.join(DATA_DIR, 'vanigargal', safeFileName(client.id) + '.json');
  writeJSON(filePath, client);
  res.json({ success: true, id: client.id });
});

app.delete('/api/vanigargal/:id', (req, res) => {
  const filePath = path.join(DATA_DIR, 'vanigargal', safeFileName(req.params.id) + '.json');
  deleteFile(filePath);
  res.json({ success: true });
});
...
app.get('/api/mathirigal', (req, res) => {
  let templates = readAllFromDir('mathirigal');
  if (templates.length === 0) {
    ...
    writeJSON(path.join(DATA_DIR, 'mathirigal', 'default.json'), defaultTpl);
...
app.post('/api/mathirigal', (req, res) => {
  const tpl = req.body;
  if (!tpl.id) tpl.id = 'tpl_' + Date.now();
  const filePath = path.join(DATA_DIR, 'mathirigal', safeFileName(tpl.id) + '.json');
  writeJSON(filePath, tpl);
  res.json({ success: true, id: tpl.id });
});

app.delete('/api/mathirigal/:id', (req, res) => {
  const filePath = path.join(DATA_DIR, 'mathirigal', safeFileName(req.params.id) + '.json');
  deleteFile(filePath);
  res.json({ success: true });
});
...
// PRODUCTS / INVENTORY
// ========================
app.get('/api/porulgal', (req, res) => {
  const products = readAllFromDir('porulgal');
  products.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
  res.json(products);
});

app.post('/api/porulgal', (req, res) => {
  const product = req.body;
  if (!product.id) product.id = 'prod_' + Date.now();
  const filePath = path.join(DATA_DIR, 'porulgal', safeFileName(product.id) + '.json');
...
// EXPENSES
// ========================
app.get('/api/selavugal', (req, res) => {
  const expenses = readAllFromDir('selavugal');
  expenses.sort((a, b) => new Date(b.date) - new Date(a.date));
  res.json(expenses);
});

app.post('/api/selavugal', (req, res) => {
  const expense = req.body;
  if (!expense.id) expense.id = 'exp_' + Date.now();
  const filePath = path.join(DATA_DIR, 'selavugal', safeFileName(expense.id) + '.json');
  writeJSON(filePath, expense);
  res.json({ success: true, id: expense.id });
});

app.delete('/api/selavugal/:id', (req, res) => {
  const filePath = path.join(DATA_DIR, 'selavugal', safeFileName(req.params.id) + '.json');
  deleteFile(filePath);
  res.json({ success: true });
});
...
// RECURRING INVOICES
// ========================
app.get('/api/thodar_pattiyalkal', (req, res) => {
  const items = readAllFromDir('thodar_pattiyalkal');
  items.sort((a, b) => (a.clientName || '').localeCompare(b.clientName || ''));
  res.json(items);
});

app.post('/api/thodar_pattiyalkal', (req, res) => {
  const item = req.body;
  if (!item.id) item.id = 'rec_' + Date.now();
  const filePath = path.join(DATA_DIR, 'thodar_pattiyalkal', safeFileName(item.id) + '.json');
  writeJSON(filePath, item);
  res.json({ success: true, id: item.id });
});

app.delete('/api/thodar_pattiyalkal/:id', (req, res) => {
  const filePath = path.join(DATA_DIR, 'thodar_pattiyalkal', safeFileName(req.params.id) + '.json');
  deleteFile(filePath);
  res.json({ success: true });
});
...
// ========================
app.get('/api/kolmudhal', (req, res) => {
  const purchases = readAllFromDir('kolmudhal');
  purchases.sort((a, b) => new Date(b.date) - new Date(a.date));
  res.json(purchases);
});

app.post('/api/kolmudhal', (req, res) => {
  const purchase = req.body;
  if (!purchase.id) purchase.id = 'pur_' + Date.now();
  const filePath = path.join(DATA_DIR, 'kolmudhal', safeFileName(purchase.id) + '.json');
  writeJSON(filePath, purchase);
  res.json({ success: true, id: purchase.id });
});

app.delete('/api/kolmudhal/:id', (req, res) => {
  const filePath = path.join(DATA_DIR, 'kolmudhal', safeFileName(req.params.id) + '.json');
  deleteFile(filePath);
  res.json({ success: true });
});
...
app.post('/api/import', (req, res) => {
  ...
  if (data.bills && Array.isArray(data.bills)) {
    for (const bill of data.bills) {
      if (bill.id) {
        writeJSON(path.join(DATA_DIR, 'pattiyalkal', safeFileName(bill.id) + '.json'), bill);
        billCount++;
      }
    }
  }
  ...
  if (data.termsTemplates && Array.isArray(data.termsTemplates)) {
    for (const tpl of data.termsTemplates) {
      if (tpl.id) {
        writeJSON(path.join(DATA_DIR, 'mathirigal', safeFileName(tpl.id) + '.json'), tpl);
        templateCount++;
      }
    }
  }
  ...
  try {
    ...
    const [pkgRes, relRes] = await Promise.all([
      ...
      fetch('https://api.github.com/repos/IamRamgarhia/Free-GST-Billing-Software/releases/latest', {
        signal: ctrl.signal,
        headers: { 'Accept': 'application/vnd.github+json', 'User-Agent': 'ElvanNiril-update-check' },
      }).catch(() => null),
    ]);
    clearTimeout(t);
...
<html><head><meta charset="utf-8"><title>Elvan Niril — building…</title>
...
function startServer(port) {
  const server = app.listen(port, '127.0.0.1', () => {
    ...
    console.log(`\n  Elvan Niril running at http://localhost:${port}`);
...
// Graceful shutdown — taskkill /f from Stop ElvanNiril.bat can interrupt a
...
function processDueRecurring() {
  ...
  const templates = readAllFromDir('thodar_pattiyalkal');
  let fired = 0;
  for (const tpl of templates) {
    ...
    try {
      ...
      writeJSON(path.join(DATA_DIR, 'pattiyalkal', safeFileName(invoiceNumber) + '.json'), bill);
      ...
      writeJSON(path.join(DATA_DIR, 'thodar_pattiyalkal', safeFileName(tpl.id) + '.json'), tpl);

      fired += 1;
    } catch (err) {
      logFatal(err, 'thodar_pattiyalkal');
    }
  }
  if (fired > 0) {

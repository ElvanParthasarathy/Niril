    ...
    return Scaffold(
      body: Stack(
        children: [
          ...
          AnimatedBuilder(
            animation: Listenable.merge([_scrollController, _navbarController]),
            builder: (context, child) {
              if (!_scrollController.hasClients) return const SizedBox.shrink();
              
              final double shrinkOffset = _scrollController.offset;
              final double statusBarHeight = MediaQuery.paddingOf(context).top;
              final double ceiling = statusBarHeight + 20.0;
              
              // 1. Hand-off logic
              final double currentTop = _kExpandedHeight - shrinkOffset - 8.0 - kToolbarHeight;
              final bool isPinned = currentTop <= ceiling;
              
              // If hand-off hasn't happened, Component B is invisible!
              if (!isPinned) return const SizedBox.shrink(); 
              
              // 2. White Pill Background Logic
              final double collisionOffset = (_kExpandedHeight + 32.0) - (ceiling + 50.0);
              final double liftStartOffset = collisionOffset - 4.0;
              ...
              return Positioned(
                ...
                child: Align(
                  ...
                  child: Opacity(
                    opacity: _navbarOpacity.value, // Fade out when bottom navbar hides
                    child: Container(
                      decoration: BoxDecoration(
          ...
          child: Stack(
            ...
            children: [
              ...
              Positioned(
                left: 24,
                right: 24,
                bottom: 40 + expandedButtonsBottom + kToolbarHeight, 
                child: Opacity(

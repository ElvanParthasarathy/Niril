...
class KooliPorulKalanjiyam implements PorulKalanjiyam {
  final KooliDatabase _db;

  KooliPorulKalanjiyam(this._db);

  PorulTharavuru _mapToDomain(KooliPorulEntry entry) {
  ...
  Stream<List<PorulTharavuru>> watchAllPorulgal() {
    return (_db.select(_db.kooliPorulTable)
  ...
  Future<List<PorulTharavuru>> getAllPorulgal() async {
    final list = await (_db.select(_db.kooliPorulTable)
          ..where((t) => t.isDeleted.equals(false))
          ..orderBy([(t) => OrderingTerm.desc(t.updatedAt)]))
        .get();
  ...
  Future<PorulTharavuru?> getPorulById(int id) async {
    final entry = await (_db.select(_db.kooliPorulTable)
  ...
  Future<int> savePorul(PorulTharavuru tharavuru) async {
    ...
    if (tharavuru.id > 0) {
      await (_db.update(_db.kooliPorulTable)..where((t) => t.id.equals(tharavuru.id)))
          .write(companion);
      return tharavuru.id;
    } else {
      return _db.into(_db.kooliPorulTable).insert(companion);
    }
  }

  @override
  Future<void> deletePorul(int id) async {
    await (_db.update(_db.kooliPorulTable)..where((t) => t.id.equals(id))).write(
  ...
  Future<void> bulkDeletePorulgal(List<int> ids) async {
    await (_db.update(_db.kooliPorulTable)..where((t) => t.id.isIn(ids))).write(
      KooliPorulTableCompanion(
        isDeleted: const Value(true),
        deletedAt: Value(DateTime.now()),
      ),
  ...
  Future<void> permanentDeletePorul(int id) async {
    await (_db.delete(_db.kooliPorulTable)..where((t) => t.id.equals(id))).go();
  }
  ...
  Future<int> purgeExpiredPorulgal({int days = 30}) async {
    ...
    return (_db.delete(_db.kooliPorulTable)
          ..where((t) => t.isDeleted.equals(true))
  ...
  Future<void> deleteAllPorulgal() async {
    await _db.delete(_db.kooliPorulTable).go();
  }
}

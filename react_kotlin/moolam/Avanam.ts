...
// Firebase stubs for Kotlin WebView build using offline JSON
import { offlineData } from './offlineData';

const rtdb = {};
const ref = (...args: any[]) => ({ path: args.length > 1 ? args[1] : '' });
const get = async (dbRef: any) => ({ exists: () => !!offlineData[dbRef.path], val: () => offlineData[dbRef.path] || null });
const set = async (...args: any[]) => {};
const remove = async (...args: any[]) => {};
const update = async (...args: any[]) => {};
const onValue = (dbRef: any, callback: any) => {
  callback({ exists: () => !!offlineData[dbRef.path], val: () => offlineData[dbRef.path] || null });
  return () => {};
};
const runTransaction = async (...args: any[]) => ({ committed: true, snapshot: { val: () => null } });

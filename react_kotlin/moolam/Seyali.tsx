...
// Firebase removed for Kotlin build
...
function Seyali() {
  ...
  // App Back Button Listener (Removed for Kotlin Bridge)
  ...
  const [dbProfile, setProfile] = useState<any>(() => {
    if (isBridge) return { niruvanathinPeyar: 'Kotlin App' };
  ...
  // Firebase auth completely removed for Kotlin build
  const firebaseUser = { uid: 'local', email: 'local@app' };
  const firebaseAuthLoading = false;
  const isProfileLoading = false;
  const [postLoginWelcomeDone, setPostLoginWelcomeDone] = useState(true);
  ...
  // Profile loading from Firebase removed for Kotlin build
  ...
  // Login/Welcome gates removed — Firebase removed for Kotlin build
